        ...
        public void TestIfItCanSendData()
        {
            ...
            using (ctx = new Context())
            {
                ...
                using (slaveSocket2 = ctx.Socket(SocketType.PULL))
                {
                    ...
                    using (var target = new SumadorMaestro(2, numeros, masterSocket, masterSocket))
                    {
        ...
        public void TestIfItCanReceiveSum()
        {
            ...
            using (ctx = new Context())
            {
                ...
                using (slaveSocket2 = ctx.Socket(SocketType.PUSH))
                {
                    ...
                    using (var target = new SumadorMaestro(2, new int[] {}, masterSocket, masterSocket))
                    {

                        // Act
                        target.CallMethod("RecibirDatos");

                        var task1 = Task.Factory.StartNew(() =>
                        {
                            slaveSocket1.Send<int>(1);
                        });
                        
                        var task2 = Task.Factory.StartNew(() =>
                        {
                            slaveSocket2.Send<int>(2);
                        });                        

                        // Assert
                        Assert.That(target.Result, Is.EqualTo(3));
